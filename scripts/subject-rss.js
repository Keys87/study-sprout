...
export function renderButtons() {
    ...
    const resourceApplicator = (resourceName, id) => {
        ...
        const element = `<button type="button" id="${id}" class="flex items-center justify-start gap-3 px-4 py-3 bg-linear-to-r from-[#dde6ef] to-[#cfd9e2] hover:from-[#f5d8cc] hover:to-[#e3c7bc] rounded-lg transition-colors">${resourceName}</button>`;
        return element;
    }
